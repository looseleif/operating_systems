...
static int prod_num = 0;
static int cons_num = 0;
...
void* producer(void *arg) {
  while (true) {
    ...
    if(item_count >= cons_num)
    {
    	//cout << "Broadcasting... items = " << item_count << endl;
    	//cout << "Cons num = " << cons_num << endl;
    	need_item_cv.broadcast();
    }
    else
    {
    	//need_item_cv.signal();
...
void* consumer(void *arg) {
  while (true) {
    ...
    if((SHARED_BUFFER_SIZE - item_count) >= prod_num)
    {
    	//cout << "Broadcasting... space = " << SHARED_BUFFER_SIZE - item_count << endl;
    	need_space_cv.broadcast();
    }
    else
    {
...
int main(int argc, char *argv[]) {
  ...
  prod_num = producer_count;
  cons_num = consumer_count;
