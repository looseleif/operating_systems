...
int uthread_create(void* (*start_routine)(void*), void* arg)
{
	...
	}else if(th->getPriority()==1){

		ready = &ORANGE_ready;
		
	}else if(th->getPriority()==0){
...
{
	cout << "leaving " << uthread_self() << endl;
 	disableInterrupts();

	if(running->getPriority()==2){
		ready = &RED_ready;
	} else if (running->getPriority()==1){
		ready = &ORANGE_ready;
	} else if(running->getPriority()==0){
		ready = &GREEN_ready;
	}
...
{
    if(_threads[tid]->getPriority()==2){
		_threads[tid]->setPriority(ORANGE);
		return 0;
	} else if(_threads[tid]->getPriority()==1){
		_threads[tid]->setPriority(RED);
		return 0;
	} else if(_threads[tid]->getPriority()==0){
		return 0;
	} else {
...
int delete_get_prior(int tid)
{
	...
	if(_threads[tid]->getPriority()==1){

		return 1;
		
	}

	if(_threads[tid]->getPriority()==0){
